...
#include "storage/processors/QueryStatsProcessor.h"
...
StorageServiceHandler::future_getOutBound(const cpp2::GetNeighborsRequest& req) {
    auto* processor = QueryBoundProcessor::instance(kvstore_, schemaMan_.get(), indexMan_.get());
...
StorageServiceHandler::future_outBoundStats(const cpp2::GetNeighborsRequest& req) {
    auto* processor = QueryStatsProcessor::instance(kvstore_, schemaMan_.get(), indexMan_.get());
    RETURN_FUTURE(processor);
}
